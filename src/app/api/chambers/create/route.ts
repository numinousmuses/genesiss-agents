...
export async function POST(request: NextRequest) {
  try {
    ...
    const newChatCommand = new PutCommand({
      TableName: Resource.ChambersChatsTable.name, // Replace with your actual chats table name
      Item: {
        chatID,
        chatTitle,
    ...
    const updateUserChatsCommand = new UpdateCommand({
      TableName: Resource.ChambersChatsTable.name, // Replace with your actual users table name
      Key: {
        userID: userId,
      },
      UpdateExpression: "SET chats = list_append(if_not_exists(chats, :emptyList), :newChatID)",
